...
abstract class Questionnaire
{
	...
	private function printPage ($fbid, $currentpage, $sessionid, $markMissing, $nextpage)
	{
		...
		echo '<form action="'.JURI::current().'?'.$u->buildQuery($params).'" method="post">';
		...
		foreach($fragen as $frage)
		{
			...
			#FIXME �berf�hren in Objektorientierung: dynamisch Funktion laden (hook-object)
			switch($fragentyp)
			{
				case 111:
					{
				...
				case 311:
					{
						...
						echo("<p style='font-weight:bold;'>$fragentext</p>");
						echo("<table style=''><tr><th/>");
						for ($i=0;$i<count($codes);$i++)
						{
							$code=$codes[$i];
							$label=$code['Label'];
							echo("<th style='padding-left: 10px; padding-right: 10px;'>$label</th>");
						}
						echo("</tr>");
	...
	{
		//klaere aktuelle, vorherige und naechste Seite
		$db =& JFactory::getDBO();
		...
		if ($seite<1) $nextpage = null; //wenn beendet, dann gibt es keine n�chste Seite
